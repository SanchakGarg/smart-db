...
export class InventoryService {
  ...
  generateQrBatch(size: "small" | "large", actor: string): RegisterQrBatchResponse {
    const perPage = size === "small" ? 120 : 90;
    const maxRow = this.db
      .prepare(`SELECT COALESCE(MAX(end_number), 0) AS n FROM qr_batches WHERE id <> 'external'`)
      .get() as { n: number };
    const startNumber = Number(maxRow.n) + 1;
    return this.registerQrBatch({
      prefix: "QR",
      startNumber,
      count: perPage,
      actor,
  ...
  assignQr(input: AssignQrCommand): InventoryEntitySummary {
    ...
    if (!qrRow) {
      throw new ConflictError(
        `QR code ${qrCodeValue} was not generated by this system. Only pre-printed labels can be assigned.`,
        { qrCode: qrCodeValue },
      );
    }
