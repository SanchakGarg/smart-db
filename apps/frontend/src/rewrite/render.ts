      ...
      <div aria-live="polite">
        ${state.scanResult?.mode === "unknown" ? `
          <div class="result-card">
            <h3>${escapeHtml(state.scanResult.code)} is not a Smart DB label</h3>
            <p>This code was not generated by Smart DB. Only pre-printed QR labels can be assigned.</p>
          </div>
        ` : ""}
...
function renderAdminTab(state: RewriteUiState): string {
  ...
  return `
    <section id="panel-admin" role="tabpanel" aria-labelledby="tab-admin">
      ...
      <section class="panel">
        ${renderPanelTitle("Print QR labels", "Generates one sheet and registers all codes in the database. New sheets never repeat old codes.")}
        ${state.latestBatch ? `
          <div class="latest-batch-card">
            <div>
              <strong>Latest batch</strong>
              <p>${escapeHtml(`${state.latestBatch.prefix}-${state.latestBatch.startNumber} to ${state.latestBatch.prefix}-${state.latestBatch.endNumber}`)}</p>
              <small>${escapeHtml(`${state.latestBatch.endNumber - state.latestBatch.startNumber + 1} labels · ${state.latestBatch.actor}`)}</small>
            </div>
            <button type="button" data-action="download-labels" ${disabled(isDownloadingLabels)}>${isDownloadingLabels ? "Downloading..." : "Re-download"}</button>
          </div>
        ` : `<p class="muted-copy">No QR batch has been generated yet.</p>`}
        <div class="qr-size-picker">
          <button type="button" class="qr-size-btn" data-action="generate-qr" data-size="large" ${disabled(state.pendingAction !== null)}>
            <strong>Large QR</strong>
            <span>6 × 15 = 90 per sheet</span>
          </button>
          <button type="button" class="qr-size-btn" data-action="generate-qr" data-size="small" ${disabled(state.pendingAction !== null)}>
            <strong>Small QR</strong>
            <span>8 × 15 = 120 per sheet</span>
          </button>
        </div>
        ${state.pendingAction === "batch" ? `<p class="muted-copy" style="text-align:center">Generating PDF…</p>` : ""}
      </section>
